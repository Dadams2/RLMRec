"""
Visualization System for VAE Embedding Dynamics
Works correctly for both VAE-only and VAE+LLM models by using the model's own encode/decode methods

This version doesn't manually replicate the forward pass, instead calls the model's methods directly.
"""
...
class EmbeddingVisualizer:
    ...
    def capture_embeddings_from_model(self, model, data_handler, n_samples=1000, batch_size=256, fixed_users=None):
        ...
        Uses the model's own encode/decode methods instead of manually replicating forward pass
        ...
        with torch.no_grad():
            for i in range(0, len(sampled_users), batch_size):
                ...
                # Stage 1: Raw LLM embeddings (for compatibility)
    ...
    def create_static_visualization(self, reduced_embeddings, user_ids, save_path='embedding_vis.png'):
        ...
        for idx, (stage, title) in enumerate(zip(stages, stage_titles)):
            ...
            if stage in reduced_embeddings:
                ...
                # Check if embeddings are essentially zero (VAE-only model)
                if np.abs(emb).max() < 1e-6 and stage in ['llm_processed']:
                    ax.text(0.5, 0.5, 'Not Used\n(VAE-only)', 
    ...
    def create_distribution_animation(self, model, data_handler, n_frames=50, n_samples=1000):
        ...
        with torch.no_grad():
            ...
            mu_llm = mu_llm.cpu().numpy()
            logvar_llm = logvar_llm.cpu().numpy()
            sigma_llm = np.exp(0.5 * logvar_llm)
            
            mu_src = mu_src.cpu().numpy()
            logvar_src = logvar_src.cpu().numpy()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Visualize VAE embedding dynamics (works for VAE-only and VAE+LLM)')
