...
class mult_vae_GODM_SUOT(BaseModel):
    def __init__(self, data_handler):
        super(mult_vae_GODM_SUOT, self).__init__(data_handler)

        self.beta = self.hyper_config['beta']
        
        # SUOT-specific hyperparameters
        self.num_projections = self.hyper_config.get('num_projections', 50)
        self.rho1 = self.hyper_config.get('rho1', 10.0)  # Marginal relaxation for source
        self.rho2 = self.hyper_config.get('rho2', 10.0)  # Marginal relaxation for target
        self.suot_niter = self.hyper_config.get('suot_niter', 10)  # Frank-Wolfe iterations
        self.suot_p = self.hyper_config.get('suot_p', 2)  # Power for cost (2 = Wasserstein-2)
        self.suot_mode = self.hyper_config.get('suot_mode', 'icdf')  # 'icdf' or 'backprop' (use icdf to avoid double backward)
    ...
    def compute_suot_distance(self, mu_src, logvar_src, mu_llm, logvar_llm):
        """
        Compute Sliced Unbalanced Optimal Transport distance between two Gaussian distributions.
        ...
        Returns:
            suot_loss: Sliced unbalanced OT distance
        """
        batch_size = mu_src.shape[0]
        latent_dim = mu_src.shape[1]
        ...
        a = torch.ones(batch_size, device=mu_src.device) / batch_size
        b = torch.ones(batch_size, device=mu_llm.device) / batch_size
        
        # Compute SUOT distance
    ...
    def cal_loss(self, user, batch_data):
        ...
        # Use SUOT distance instead of simple Wasserstein-2
        SUOT = self.compute_suot_distance(mu_src, logvar_src, mu_llm, logvar_llm)

        KLD = KLD + self.beta * SUOT

        loss = BCE + KLD
        losses = {'rec_loss': BCE, 'reg_loss': KLD, 'suot_loss': SUOT}
        return loss, losses
